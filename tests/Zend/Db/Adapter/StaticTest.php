...
{
    protected static $_isCaseSensitiveFileSystem;
    ...
    public function testDbFactoryAlternateNamespaceExceptionInvalidAdapter()
    {
        ...
        try {
            $db = Zend_Db::factory('Db', ['dbname' => 'dummy', 'adapterNamespace' => 'Zend']);
            set_include_path($ip);
            ...
            $this->assertEquals("Adapter class 'Zend_Db' does not extend Zend_Db_Adapter_Abstract", $e->getMessage());
        }
    }
    ...
    {
        if (null === self::$_isCaseSensitiveFileSystem) {
