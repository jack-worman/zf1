...
class Zend_Amf_Parse_Amf0_Serializer extends Zend_Amf_Parse_Serializer
{
    ...
    protected function writeObjectReference(&$object, $markerType, $objectByVal = false)
    {
        // Workaround for PHP5 with E_STRICT enabled complaining about "Only
        // variables should be passed by reference"
        if ((null === $object) && (false !== $objectByVal)) {
            $object = &$objectByVal;
        }

        if (Zend_Amf_Constants::AMF0_OBJECT == $markerType
            || Zend_Amf_Constants::AMF0_MIXEDARRAY == $markerType
            || Zend_Amf_Constants::AMF0_ARRAY == $markerType
            || Zend_Amf_Constants::AMF0_TYPEDOBJECT == $markerType
        ) {
    ...
    public function writeObject($object)
    {
        ...
        foreach ($object as $key => &$value) {
            ...
            if (is_string($key) && 0 === strpos((string) $key, '_')) {
                continue;
            }
