...
class Zend_Locale
{
    ...
    {
        if ((true === self::$compatibilityMode) or (func_num_args() > 0)) {
            if (!self::$_breakChain) {
    ...
    {
        if (('auto' === $locale) or ('root' === $locale) or ('default' === $locale)
            or ('environment' === $locale) or ('browser' === $locale)) {
        ...
        if (true === isset(self::$_localeData[(string) $locale])) {
            self::$_default = [(string) $locale => $quality];
        } else {
            $elocale = explode('_', (string) $locale);
            if (true === isset(self::$_localeData[$elocale[0]])) {
    ...
    public static function getEnvironment()
    {
        ...
        foreach ($languages as $locale) {
            ...
            if ('C' !== $language) {
                ...
                if (true === isset(self::$_localeData[$language])) {
                    $languagearray[$language] = 1;
                    if (false !== strpos((string) $language, '_')) {
    ...
    public static function getBrowser()
    {
        ...
        foreach ($accepted as $accept) {
            ...
            if ((false === isset($languages[$region])) || ($languages[$region] < $quality)) {
    ...
    public function setLocale($locale = null)
    {
        ...
        if (false === isset(self::$_localeData[(string) $locale])) {
            // Is it an alias? If so, we can use this locale
            if (true === isset(self::$_localeAliases[$locale])) {
                $this->_locale = $locale;
            ...
            if (true === isset($region[2])) {
                if (('_' === $region[2]) or ('-' === $region[2])) {
                    $region = substr((string) $region, 0, 2);
                }
            }

            if (true === isset(self::$_localeData[(string) $region])) {
                $this->_locale = $region;
            } else {
                $this->_locale = 'root';
            }
    ...
    public static function getHttpCharset()
    {
        ...
        $charsets = [];
        if (false === $httpcharsets) {
            return $charsets;
        }

        $accepted = preg_split('/,\s*/', $httpcharsets);
        foreach ($accepted as $accept) {
            if (true === empty($accept)) {
                continue;
            }

            if (false !== strpos((string) $accept, ';')) {
    ...
    public static function isLocale($locale, $strict = false, $compatible = true)
    {
        ...
        if ((null === $locale) || (!is_string($locale) and !is_array($locale))) {
            return false;
        }
        ...
        if ((true === $compatible) and (true === self::$compatibilityMode)) {
            trigger_error('You are running Zend_Locale in compatibility mode... please migrate your scripts', E_USER_NOTICE);
            if (true === isset(self::$_localeData[$locale])) {
                return $locale;
            } elseif (!$strict) {
                $locale = explode('_', $locale);
                if (true === isset(self::$_localeData[$locale[0]])) {
                    return $locale[0];
                }
            }
        } else {
    ...
    private static function _prepareLocale($locale, $strict = false)
    {
        ...
        if (!isset(self::$_localeData[$parts[0]])) {
            if ((1 == count($parts)) && array_key_exists($parts[0], self::$_territoryData)) {
