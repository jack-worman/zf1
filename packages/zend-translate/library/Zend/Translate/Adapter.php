...
abstract class Zend_Translate_Adapter
{
    ...
    public function addTranslation($options = [])
    {
        ...
        if (is_string($options['content']) and is_dir($options['content'])) {
            ...
            foreach ($iterator as $directory => $info) {
                ...
                if ($info->isDir()) {
                    // pathname as locale
                    if ((self::LOCALE_DIRECTORY === $options['scan']) and Zend_Locale::isLocale($file, true, false)) {
                        $options['locale'] = $file;
                        $prev = (string) $options['locale'];
                    }
                } elseif ($info->isFile()) {
                    // filename as locale
                    if (self::LOCALE_FILENAME === $options['scan']) {
        ...
        if ((true === isset($this->_translate[$originate])) and (count($this->_translate[$originate]) > 0)) {
    ...
    public function setOptions(array $options = [])
    {
        ...
        if (isset(self::$_cache) and (true == $change)) {
            $id = 'Zend_Translate_'.$this->toString().'_Options';
            if (self::$_cacheTags) {
    ...
    public function getOptions($optionKey = null)
    {
        ...
        if (true === isset($this->_options[$optionKey])) {
    ...
    public function getMessages($locale = null)
    {
        ...
        if ((true === empty($locale)) or (false === $this->isAvailable($locale))) {
            $locale = $this->_options['locale'];
        }
    ...
    {
        if ((false !== $original) and (true !== $original)) {
            $locale = $original;
            $original = false;
        }
        ...
        if ((is_string($messageId) || is_int($messageId)) && isset($this->_translate[$locale][$messageId])) {
            // return original translation
            return true;
        } elseif ((2 != strlen((string) $locale)) and (false === $original)) {
            // faster than creating a new locale and separate the leading part
            $locale = substr((string) $locale, 0, -strlen((string) strrchr($locale, '_')));
