...
class Zend_Http_Client_Adapter_Curl implements Zend_Http_Client_Adapter_Interface, Zend_Http_Client_Adapter_Stream
{
    ...
    public function write($method, $uri, $httpVersion = 1.1, $headers = [], $body = '')
    {
        ...
        $curlHttp = (1.1 == $httpVersion) ? CURL_HTTP_VERSION_1_1 : CURL_HTTP_VERSION_1_0;
        ...
        } elseif (Zend_Http_Client::PUT == $method) {
            // This is a PUT by a setRawData string, not by file-handle
            curl_setopt($this->_curl, CURLOPT_POSTFIELDS, $body);
        } elseif (Zend_Http_Client::PATCH == $method) {
            // This is a PATCH by a setRawData string
            curl_setopt($this->_curl, CURLOPT_POSTFIELDS, $body);
        } elseif (Zend_Http_Client::DELETE == $method) {
            // This is a DELETE by a setRawData string
            curl_setopt($this->_curl, CURLOPT_POSTFIELDS, $body);
        } elseif (Zend_Http_Client::OPTIONS == $method) {
        ...
        if (false !== stripos($this->_response, "HTTP/1.0 200 Connection established\r\n\r\n")) {
