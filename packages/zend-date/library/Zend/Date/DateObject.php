...
abstract class Zend_Date_DateObject
{
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if ((1901 < $year) and ($year < 2038)) {
            ...
            if (($this->_timezone != $oldzone) and (false === $gmt)) {
    ...
    protected static function isYearLeapYear($year)
    {
        ...
        if (0 == $year % 400) {
            return true;
        } elseif (($year > 1582) and (0 == $year % 100)) {
            return false;
        }
        ...
        for ($i = 0; $i < $length; ++$i) {
            switch ($format[$i]) {
                ...
                case 'S':  // english suffix for day of month, st nd rd th
                    if (($date['mday'] % 10) == 1) {
                        $output .= 'st';
                    } elseif ((($date['mday'] % 10) == 2) and (12 != $date['mday'])) {
                        $output .= 'nd';
                ...
                case 'O':  // difference to GMT in hours
                    $gmtstr = (true === $gmt) ? 0 : $this->getGmtOffset();
                ...
                case 'Z':  // timezone offset in seconds
                    $output .= (true === $gmt) ? 0 : -$this->getGmtOffset();
                    break;
    ...
    protected static function dayOfWeek($year, $month, $day)
    {
        ...
        $correction = 0;
        if (($year < 1582) or ((1582 == $year) and (($month < 10) or ((10 == $month) && ($day < 15))))) {
            $correction = 3;
        }
    ...
    protected function weekNumber($year, $month, $day)
    {
        ...
        if ((1 == $month) and (($firstday < 1) or ($firstday > 4)) and ($day < 4)) {
            $firstday = self::dayOfWeek($year - 1, 1, 1);
            $month = 12;
            $day = 31;
        } elseif ((12 == $month) and ((self::dayOfWeek($year + 1, 1, 1) < 5)
