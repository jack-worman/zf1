...
class Zend_Date extends Zend_Date_DateObject
{
    ...
    public function __construct($date = null, $part = null, $locale = null)
    {
        ...
        if ((null !== $date) and !is_array($date) and !($date instanceof Zend_TimeSync_Protocol)
            and !($date instanceof Zend_Date) and !defined($date) and Zend_Locale::isLocale($date, true, false)) {
            $locale = $date;
            $date = null;
            $part = null;
        } elseif ((null !== $part) and !defined($part) and Zend_Locale::isLocale($part, true, false)) {
            $locale = $part;
            $part = null;
        }

        $this->setLocale($locale);
        if (is_string($date) && (null === $part) && (strlen((string) $date) <= 5)) {
            $part = $date;
            $date = null;
        }
        ...
        if ($date instanceof Zend_TimeSync_Protocol) {
            ...
            $part = null;
        } elseif (0 != parent::$_defaultOffset) {
        ...
        // set datepart
        if ((null !== $part && self::TIMESTAMP !== $part) or (!is_numeric($date))) {
    ...
    public static function setOptions(array $options = [])
    {
        ...
        foreach ($options as $name => $value) {
            ...
            if (array_key_exists($name, self::$_options)) {
                switch ($name) {
                    case 'format_type':
                        if (('php' != strtolower((string) $value)) && ('iso' != strtolower((string) $value))) {
    ...
    public function toString($format = null, $type = null, $locale = null)
    {
        ...
        if ((null !== $format) && !defined($format)
            && ('ee' != $format) && ('ss' != $format) && ('GG' != $format) && ('MM' != $format) && ('EE' != $format) && ('TT' != $format)
            && Zend_Locale::isLocale($format, null, false)) {
            $locale = $format;
            $format = null;
        }

        if ((null !== $type) and ('php' != $type) and ('iso' != $type)
        ...
        } elseif ((('php' == self::$_options['format_type']) && (null === $type)) or ('php' == $type)) {
    ...
    public function get($part = null, $locale = null)
    {
        ...
        if ((null !== $part) && !defined($part)
            && ('ee' != $part) && ('ss' != $part) && ('GG' != $part) && ('MM' != $part) && ('EE' != $part) && ('TT' != $part)
        ...
        if (null === $part) {
            $part = self::TIMESTAMP;
        } elseif ('php' == self::$_options['format_type']) {
    ...
    private function _parseIsoToDate($token, $locale)
    {
        switch ($token) {
            ...
            case 'w':
                ...
                return $this->_toComment(('0' == $week[0]) ? $week[1] : $week);
                break;
            ...
            case self::ERA_NARROW:
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return $this->_toComment(iconv_substr((string) Zend_Locale_Data::getContent($locale, 'era', ['gregorian', 'Abbr', '0']), 0, 1, 'UTF-8')).'.';
                }

                return $this->_toComment(iconv_substr((string) Zend_Locale_Data::getContent($locale, 'era', ['gregorian', 'Abbr', '1']), 0, 1, 'UTF-8')).'.';
                break;
        ...
        switch ($token[0]) {
            case 'y':
                if ((4 == strlen((string) $token)) && (abs($this->getUnixTimestamp()) <= 0x7FFFFFFF)) {
                    return 'Y';
                }
    ...
    {
        if ('php' == self::$_options['format_type']) {
            $part = Zend_Locale_Format::convertPhpToIsoFormat($part);
        }
    ...
    private function _assign($calc, $date, $comp = 0, $dst = false)
    {
        ...
        if ((true === self::$_options['fix_dst']) and (false !== $dst) and (true === $this->_dst)) {
    ...
    private function _calculate($calc, $date, $part, $locale)
    {
        ...
        if ((null !== $part) && (2 !== strlen((string) $part)) && Zend_Locale::isLocale($part, null, false)) {
            $locale = $part;
            $part = null;
        }
        ...
        if (true === is_array($date)) {
            if (false === empty($part)) {
                switch ($part) {
                    ...
                    case self::WEEKDAY_NARROW:
                    case self::WEEKDAY_NAME:
                        if (true === isset($date['weekday'])) {
                            $date = $date['weekday'];
                            $part = self::WEEKDAY_DIGIT;
                        }
                        break;
                    case self::DAY_OF_YEAR:
                        if (true === isset($date['day_of_year'])) {
                    ...
                    case self::TIMEZONE:
                    case self::TIMEZONE_NAME:
                        if (true === isset($date['timezone'])) {
                            $date = $date['timezone'];
                        }
                        break;
                    case self::TIMESTAMP:
                        if (true === isset($date['timestamp'])) {
                            $date = $date['timestamp'];
                        }
                        break;
                    case self::WEEK:
                        if (true === isset($date['week'])) {
                            $date = $date['week'];
                        }
                        break;
                    case self::TIMEZONE_SECS:
                        if (true === isset($date['gmtsecs'])) {
                            $date = $date['gmtsecs'];
                        }
                        break;
                    default:
        ...
        switch ($part) {
            ...
            case self::MONTH_NAME:
                ...
                if ($cnt < 12) {
                    ...
                    if ('add' == $calc) {
                        $date += $found;
                        $calc = 'set';
                        if (false == self::$_options['extend_month']) {
            ...
            case self::ISO_8601:
                ...
                if (empty($datematch)) {
                    ...
                    $datematch[2] = 1;
                    $datematch[3] = 1;
                } elseif (2 == iconv_strlen($datematch[1], 'UTF-8')) {
            ...
            default:
                if (!is_numeric($date) || !empty($part)) {
                    try {
                        ...
                        if ((false !== strpos((string) strtoupper((string) $part), 'YY')) and (false === strpos((string) strtoupper((string) $part), 'YYYY'))) {
                            $parsed['year'] = self::getFullYear($parsed['year']);
                        }
    ...
    private function _time($calc, $time, $format, $locale)
    {
        ...
        if ($time instanceof Zend_Date) {
            ...
            if (is_array($time)) {
                if ((true === isset($time['hour'])) or (true === isset($time['minute']))
                    or (true === isset($time['second']))) {
                    $parsed = $time;
                } else {
                    // require_once 'Zend/Date/Exception.php';
    ...
    private function _date($calc, $date, $format, $locale)
    {
        ...
        if ($date instanceof Zend_Date) {
            ...
            if (is_array($date)) {
                if ((true === isset($date['year'])) or (true === isset($date['month']))
                    or (true === isset($date['day']))) {
                    $parsed = $date;
                } else {
                    // require_once 'Zend/Date/Exception.php';
                ...
                if (('php' == self::$_options['format_type']) && !defined($format)) {
                    $format = Zend_Locale_Format::convertPhpToIsoFormat($format);
                }
                try {
                    if (null === $locale) {
                    ...
                    if ((false !== strpos((string) strtoupper((string) $format), 'YY')) and (false === strpos((string) strtoupper((string) $format), 'YYYY'))) {
                        $parsed['year'] = self::getFullYear($parsed['year']);
                    }
                } catch (Zend_Locale_Exception $e) {
                    // require_once 'Zend/Date/Exception.php';
    ...
    private function _calcvalue($calc, $value, $type, $parameter, $locale)
    {
        ...
        } elseif (!is_array($value) && !is_numeric($value) && ('iso' != $type) && ('arpa' != $type)) {
            // require_once 'Zend/Date/Exception.php';
    ...
    public static function isDate($date, $format = null, $locale = null)
    {
        ...
        if ((null !== $format) && ('ee' != $format) && ('ss' != $format) && ('GG' != $format) && ('MM' != $format) && ('EE' != $format) && ('TT' != $format)
            && Zend_Locale::isLocale($format, null, false)) {
            $locale = $format;
            $format = null;
        }
        ...
        } elseif (('php' == self::$_options['format_type']) && !defined($format)) {
            $format = Zend_Locale_Format::convertPhpToIsoFormat($format);
        }
        ...
        if (((false !== strpos((string) $format, 'Y')) or (false !== strpos((string) $format, 'y')))
            and (!isset($parsed['year']))) {
            // Year expected but not found
            return false;
        }

        if ((false !== strpos((string) $format, 'M')) and (!isset($parsed['month']))) {
            // Month expected but not found
            return false;
        }

        if ((false !== strpos((string) $format, 'd')) and (!isset($parsed['day']))) {
            // Day expected but not found
            return false;
        }

        if (((false !== strpos((string) $format, 'H')) or (false !== strpos((string) $format, 'h')))
            and (!isset($parsed['hour']))) {
            // Hour expected but not found
            return false;
        }

        if ((false !== strpos((string) $format, 'm')) and (!isset($parsed['minute']))) {
            // Minute expected but not found
            return false;
        }

        if ((false !== strpos((string) $format, 's')) and (!isset($parsed['second']))) {
            // Second expected  but not found
            return false;
        }

        // Set not given dateparts
        if (false === isset($parsed['hour'])) {
            $parsed['hour'] = 12;
        }

        if (false === isset($parsed['minute'])) {
            $parsed['minute'] = 0;
        }

        if (false === isset($parsed['second'])) {
            $parsed['second'] = 0;
        }

        if (false === isset($parsed['month'])) {
            $parsed['month'] = 1;
        }

        if (false === isset($parsed['day'])) {
            $parsed['day'] = 1;
        }

        if (false === isset($parsed['year'])) {
            $parsed['year'] = 1970;
        }
