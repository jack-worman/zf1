...
class Zend_Search_Lucene_Search_QueryLexer extends Zend_Search_Lucene_FSM
{
    ...
    {
        if (false !== strpos((string) self::QUERY_WHITE_SPACE_CHARS, $char)) {
            return self::IN_WHITE_SPACE;
        } elseif (false !== strpos((string) self::QUERY_SYNT_CHARS, $char)) {
            return self::IN_SYNT_CHAR;
        } elseif (false !== strpos((string) self::QUERY_MUTABLE_CHARS, $char)) {
            return self::IN_MUTABLE_CHAR;
        } elseif (false !== strpos((string) self::QUERY_LEXEMEMODIFIER_CHARS, $char)) {
            return self::IN_LEXEME_MODIFIER;
        } elseif (false !== strpos((string) self::QUERY_ASCIIDIGITS_CHARS, $char)) {
    ...
    public function tokenize($inputString, $encoding)
    {
        ...
        if (self::ST_WHITE_SPACE != $this->getState()) {
    ...
    public function addQuerySyntaxLexeme()
    {
        ...
        if (false !== strpos((string) self::QUERY_DOUBLECHARLEXEME_CHARS, $lexeme)) {
        ...
        if (Zend_Search_Lucene_Search_QueryToken::TT_FIELD_INDICATOR == $token->type) {
            $token = array_pop($this->_lexemes);
            if (null === $token || Zend_Search_Lucene_Search_QueryToken::TT_WORD != $token->type) {
                // require_once 'Zend/Search/Lucene/Search/QueryParserException.php';
