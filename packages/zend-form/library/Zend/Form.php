...
class Zend_Form implements Iterator, Countable, Zend_Validate_Interface
{
    ...
    public function setOptions(array $options)
    {
        ...
        foreach ($options as $key => $value) {
            ...
            if (method_exists($this, $method)) {
                if ('View' == $normalized && !($value instanceof Zend_View_Interface)) {
                    continue;
                }
                $this->$method($value);
            } else {
    ...
    {
        $unset = &$array;
        $path = \trim((string) strtr((string) (string) $arrayPath, ['[' => '/', ']' => '']), '/');
    ...
    public function __call($method, $args)
    {
        if ('render' == substr((string) $method, 0, 6)) {
            ...
            if (false !== ($decorator = $this->getDecorator($decoratorName))) {
                ...
                if ('FormElements' === $decoratorName
                    || 'PrepareElements' === $decoratorName) {
                    $this->_setIsRendered();
                }
    ...
    public function valid()
    {
        ...
        return false !== current($this->_order);
    }
