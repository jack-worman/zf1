...
class Zend_Pdf_Parser
{
    ...
    private function _loadXRefTable($offset)
    {
        ...
        if ('xref' == $nextLexeme) {
            ...
            if ('XRef' != $trailerDict->Type->value) {
                // require_once 'Zend/Pdf/Exception.php';
            ...
            if (null === $trailerDict->W || Zend_Pdf_Element::TYPE_ARRAY != $trailerDict->W->getType()) {
                // require_once 'Zend/Pdf/Exception.php';
            ...
            if (0 == $entryField2Size || 0 == $entryField3Size) {
                // require_once 'Zend/Pdf/Exception.php';
            ...
            if (null !== $trailerDict->Index) {
                if (Zend_Pdf_Element::TYPE_ARRAY != $trailerDict->Index->getType()) {
                    // require_once 'Zend/Pdf/Exception.php';
            ...
            for ($count = 0; $count < $sections; ++$count) {
                ...
                for ($count2 = 0; $count2 < $entries; ++$count2) {
                    if (0 == $entryField1Size) {
                        $type = 1;
                    } elseif (1 == $entryField1Size) { // Optimyze one-byte field case
                    ...
                    if (1 == $entryField2Size) { // Optimyze one-byte field case
                    ...
                    if (1 == $entryField3Size) { // Optimyze one-byte field case
    ...
    public function __construct($source, Zend_Pdf_ElementFactory_Interface $factory, $load)
    {
        ...
        if ('%PDF-' != substr((string) $pdfVersionComment, 0, 5)) {
            // require_once 'Zend/Pdf/Exception.php';
        ...
        if (false === $this->_stringParser->offset
