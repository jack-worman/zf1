...
class Zend_Pdf_Element_Object_Stream extends Zend_Pdf_Element_Object
{
     ...
     * @var array|null
     */
    private $_initialDictionaryData;
    ...
    private function _extractDictionaryData()
    {
        ...
        if (null === $this->_dictionary->Filter) {
            // Do nothing.
        } elseif (Zend_Pdf_Element::TYPE_ARRAY == $this->_dictionary->Filter->getType()) {
            foreach ($this->_dictionary->Filter->items as $id => $filter) {
                $dictionaryArray['Filter'][$id] = $filter->value;
                $dictionaryArray['DecodeParms'][$id] = [];

                if (null !== $this->_dictionary->DecodeParms) {
                    if (null !== $this->_dictionary->DecodeParms->items[$id]
                        && null !== $this->_dictionary->DecodeParms->items[$id]->value) {
        ...
        } elseif (Zend_Pdf_Element::TYPE_NULL != $this->_dictionary->Filter->getType()) {
        ...
        if (null !== $this->_dictionary->F) {
            $dictionaryArray['F'] = $this->_dictionary->F->value;
        }

        $dictionaryArray['FFilter'] = [];
        $dictionaryArray['FDecodeParms'] = [];
        if (null === $this->_dictionary->FFilter) {
            // Do nothing.
        } elseif (Zend_Pdf_Element::TYPE_ARRAY == $this->_dictionary->FFilter->getType()) {
            foreach ($this->_dictionary->FFilter->items as $id => $filter) {
                ...
                if (null !== $this->_dictionary->FDecodeParms) {
                    if (null !== $this->_dictionary->FDecodeParms->items[$id]
                        && null !== $this->_dictionary->FDecodeParms->items[$id]->value) {
    ...
    {
        if (null === $this->_initialDictionaryData) {
    ...
    public function __get($property)
    {
        if ('dictionary' == $property) {
             ...
             */
            if ((!$this->_streamDecoded) && (null === $this->_initialDictionaryData)) {
                $this->_initialDictionaryData = $this->_extractDictionaryData();
            }
    ...
    public function dump(Zend_Pdf_ElementFactory $factory)
    {
        ...
        if ($this->_streamDecoded) {
            $this->_initialDictionaryData = $this->_extractDictionaryData();
            $this->_encodeStream();
        } elseif (null != $this->_initialDictionaryData) {
