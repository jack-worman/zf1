...
abstract class Zend_Pdf_Target
{
    ...
    public static function load(Zend_Pdf_Element $resource)
    {
        ...
        if (Zend_Pdf_Element::TYPE_DICTIONARY == $resource->getType()) {
            if ((null === $resource->Type || 'Action' == $resource->Type->value) && null !== $resource->S) {
                // It's a well-formed action, load it
                // require_once 'Zend/Pdf/Action.php';
                return Zend_Pdf_Action::load($resource);
            } elseif (null !== $resource->D) {
                // It's a destination
                $resource = $resource->D;
            } else {
        ...
        if (Zend_Pdf_Element::TYPE_ARRAY == $resource->getType()
            || Zend_Pdf_Element::TYPE_NAME == $resource->getType()
            || Zend_Pdf_Element::TYPE_STRING == $resource->getType()) {
