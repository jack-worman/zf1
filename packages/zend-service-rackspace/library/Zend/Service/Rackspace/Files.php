        ...
        switch ($status) {
            case '204': // break intentionally omitted
                ...
                foreach ($headers as $type => $value) {
                    if (false !== strpos((string) $type, $headerName)) {
    ...
    public function getInfoCdnContainer($container)
    {
        ...
        switch ($status) {
            case '204': // break intentionally omitted
                ...
                $data['cdn_enabled'] = ('false' !== strtolower((string) $result->getHeader(self::CDN_ENABLED)));
                $data['log_retention'] = ('false' !== strtolower((string) $result->getHeader(self::CDN_LOG_RETENTION)));
