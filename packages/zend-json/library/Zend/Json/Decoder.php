...
class Zend_Json_Decoder
{
    ...
    protected function _decodeObject()
    {
        ...
        while ($tok && self::RBRACE != $tok) {
            if (self::DATUM != $tok || !is_string($this->_tokenValue)) {
                // require_once 'Zend/Json/Exception.php';
    ...
    protected function _getNextToken()
    {
        ...
        switch ($str[$i]) {
            ...
            case '"':
                ...
                break;
            case 't':
                if (($i + 3) < $str_length && 'true' === substr((string) $str, $start, 4)) {
                    $this->_token = self::DATUM;
                }
                ...
                break;
            case 'f':
                if (($i + 4) < $str_length && 'false' === substr((string) $str, $start, 5)) {
                    $this->_token = self::DATUM;
                }
                ...
                break;
            case 'n':
                if (($i + 3) < $str_length && 'null' === substr((string) $str, $start, 4)) {
                    $this->_token = self::DATUM;
                }
                $this->_tokenValue = null;
                $i += 3;
