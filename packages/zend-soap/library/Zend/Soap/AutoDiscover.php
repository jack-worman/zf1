...
class Zend_Soap_AutoDiscover implements Zend_Server_Interface
{
    ...
    protected function _addFunctionToWsdl($function, $wsdl, $port, $binding)
    {
        ...
        $args = [];
        if ('document' == $this->_bindingStyle['style']) {
        ...
        if ('void' == $prototype->getReturnType()) {
        ...
        if (false == $isOneWayMessage) {
            ...
            if ('document' == $this->_bindingStyle['style']) {
                ...
                // Add the wrapper element part, which must be named 'parameters'
                $args['parameters'] = ['element' => $wsdl->addElement($element)];
            } elseif ('void' != $prototype->getReturnType()) {
        ...
        if ('rpc' == $this->_bindingStyle['style'] && !isset($this->_operationBodyStyle['namespace'])) {
