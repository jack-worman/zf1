...
class Zend_Ldap_Node extends Zend_Ldap_Node_Abstract implements Iterator, RecursiveIterator
{
    ...
    protected function _loadData(array $data, $fromDataSource)
    {
        ...
        if (true === $fromDataSource) {
            $this->_originalData = $data;
        } else {
            $this->_originalData = [];
        }
        $this->_children = null;
        $this->_markAsNew((true === $fromDataSource) ? false : true);
    ...
    public static function fromArray(array $data, $fromDataSource = false)
    {
        ...
        $fromDataSource = (true === $fromDataSource) ? true : false;
    ...
    {
        $this->_new = (false === $new) ? false : true;
    }
    ...
    {
        $this->_delete = (true === $delete) ? true : false;
    }
    ...
    {
        return (null === $this->_newDn) ? parent::_getDn() : $this->_newDn;
    }
    ...
    public function getChanges()
    {
        ...
        foreach ($this->_currentData as $key => $value) {
            ...
            } elseif (0 === count($this->_originalData[$key]) && !empty($value)) {
                $changes['add'][$key] = $value;
            } elseif ($this->_originalData[$key] !== $this->_currentData[$key]) {
                if (empty($value)) {
