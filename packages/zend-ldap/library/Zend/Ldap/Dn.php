...
class Zend_Ldap_Dn implements ArrayAccess
{
    ...
    public function toArray($caseFold = null)
    {
        ...
        if (self::ATTR_CASEFOLD_NONE === $caseFold) {
            return $this->_dn;
        } else {
            ...
            switch ($state) {
                case 1: // collect key
                    if ('=' === $ch) {
                        ...
                        if (self::ATTR_CASEFOLD_LOWER == $caseFold) {
                            $key = strtolower((string) $key);
                        } elseif (self::ATTR_CASEFOLD_UPPER == $caseFold) {
                ...
                case 2: // collect value
                    if ('\\' === $ch) {
                        ...
                        if ('+' === $ch && false === $multi) {
                            ...
                            $ka[] = [$lastKey];
                            $va[] = [$lastVal];
                            $multi = [strtolower((string) $lastKey)];
                        } elseif (',' === $ch || ';' === $ch || 0 === $ch) {
                            $multi = false;
                        }
                    } elseif ('=' === $ch) {
