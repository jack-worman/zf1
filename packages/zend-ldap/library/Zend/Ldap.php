...
class Zend_Ldap
{
    ...
    {
        if (!is_resource($this->_resource) || false === $this->_boundUser) {
            $this->bind();
        }
            ...
            if (array_key_exists($key, $options)) {
                ...
                switch ($key) {
                    ...
                    case 'tryUsernameSplit':
                        $permittedOptions[$key] = (true === $val
                                || '1' === $val || 0 == strcasecmp($val, 'true'));
                        break;
                    default:
    ...
    protected function _isPossibleAuthority($dname)
    {
        ...
        $accountDomainName = $this->_getAccountDomainName();
        $accountDomainNameShort = $this->_getAccountDomainNameShort();
        if (null === $accountDomainName && null === $accountDomainNameShort) {
            return true;
        }
        if (0 == strcasecmp($dname, $accountDomainName)) {
            return true;
        }
        if (0 == strcasecmp($dname, $accountDomainNameShort)) {
            return true;
        }
    ...
    public function bind($username = null, $password = null)
    {
        ...
        if (null !== $username && '' === $password && true !== $this->_getAllowEmptyPassword()) {
            /**
             * @see Zend_Ldap_Exception
             */
            // require_once 'Zend/Ldap/Exception.php';
            ...
            $message = (null === $username) ? $this->_connectString : $username;
            /*
             * @see Zend_Ldap_Exception
             */
            // require_once 'Zend/Ldap/Exception.php';
    ...
    public function count($filter, $basedn = null, $scope = self::SEARCH_SCOPE_SUB)
    {
        try {
            ...
            if (Zend_Ldap_Exception::LDAP_NO_SUCH_OBJECT === $e->getCode()) {
                return 0;
            } else {
                throw $e;
            }
    ...
    {
        return 1 == $this->count('(objectClass=*)', $dn, self::SEARCH_SCOPE_BASE);
    }
    ...
    public function copy($from, $to, $recursively = false)
    {
        ...
        if (true === $recursively && $this->countChildren($from) > 0) {
