...
class Zend_Cache_Backend_Static extends Zend_Cache_Backend implements Zend_Cache_Backend_Interface
{
    ...
    public function test($id)
    {
        ...
        if (null === $this->_tagged && $tagged = $this->getInnerCache()->load(self::INNER_CACHE_NAME)) {
            $this->_tagged = $tagged;
        } elseif (!$this->_tagged) {
            return false;
        }
    ...
    public function clean($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = [])
    {
        ...
        switch ($mode) {
            ...
            case Zend_Cache::CLEANING_MODE_ALL:
                if (null === $this->_tagged) {
                    $tagged = $this->getInnerCache()->load(self::INNER_CACHE_NAME);
                    $this->_tagged = $tagged;
                }
                if (null === $this->_tagged || empty($this->_tagged)) {
                    return true;
                }
                $urls = array_keys($this->_tagged);
                foreach ($urls as $url) {
    ...
    protected function _verifyPath($path)
    {
        ...
        return 0 !== strncmp($path, $base, strlen((string) $base));
    }
    ...
    protected static function _validateIdOrTag($string)
    {
        ...
        if ('internal-' == substr((string) $string, 0, 9)) {
            return;
        }
