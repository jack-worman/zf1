...
class Zend_OpenId_Consumer
{
    ...
    public function verify($params, &$identity = '', $extensions = null)
    {
        ...
        if (isset($params['openid_ns'])
            && Zend_OpenId::NS_2_0 == $params['openid_ns']) {
            $version = 2.0;
        }
        ...
        if ($this->_storage->getAssociationByHandle(
            ...
            if (isset($ret['is_valid']) && 'true' == $ret['is_valid']) {
                if (!Zend_OpenId_Extension::forAll($extensions, 'parseResponse', $params)) {
                    $this->_setError('Extension::parseResponse failure');
    ...
    protected function _httpRequest($url, $method = 'GET', array $params = [], &$status = null)
    {
        ...
        if (200 == $status || (400 == $status && !empty($body))) {
            return $body;
        } else {
    ...
    protected function _associate($url, $version, $priv_key = null)
    {
        ...
        while (1) {
            ...
            if ($bad_response && false !== strpos((string) $ret, 'Unknown session type')) {
                $r['error_code'] = 'unsupported-type';
            }
            $ret = $r;

            if (isset($ret['error_code'])
                && 'unsupported-type' == $ret['error_code']) {
                if ('DH-SHA256' == $params['openid.session_type']) {
                    $params['openid.session_type'] = 'DH-SHA1';
                    $params['openid.assoc_type'] = 'HMAC-SHA1';
                } elseif ('DH-SHA1' == $params['openid.session_type']) {
        ...
        if ('HMAC-SHA1' == $ret['assoc_type']) {
            $macFunc = 'sha1';
        } elseif ('HMAC-SHA256' == $ret['assoc_type']
            && $version >= 2.0) {
            $macFunc = 'sha256';
        } else {
        ...
        if ('sha1' == $macFunc) {
            if (20 != Zend_OpenId::strlen((string) $secret)) {
                $this->_setError('The length of the sha1 secret must be 20');

                return false;
            }
        } elseif ('sha256' == $macFunc) {
            if (32 != Zend_OpenId::strlen((string) $secret)) {
    ...
    protected function _discovery(&$id, &$server, &$version)
    {
        ...
        if (200 != $status || !is_string($response)) {
            return false;
        }
