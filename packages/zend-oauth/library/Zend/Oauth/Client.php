...
class Zend_Oauth_Client extends Zend_Http_Client
{
    ...
    public function prepareOauth()
    {
        ...
        $query = null;
        if (Zend_Oauth::REQUEST_SCHEME_HEADER == $requestScheme) {
        ...
        } elseif (Zend_Oauth::REQUEST_SCHEME_POSTBODY == $requestScheme) {
            if (self::GET == $requestMethod) {
            ...
            $this->paramsPost = [];
        } elseif (Zend_Oauth::REQUEST_SCHEME_QUERYSTRING == $requestScheme) {
    ...
    protected function _getSignableParametersAsQueryString()
    {
        ...
        if (self::ENC_FORMDATA != $this->enctype && !empty($this->paramsPost)) {
            $params = array_merge($params, $this->paramsPost);
        }
