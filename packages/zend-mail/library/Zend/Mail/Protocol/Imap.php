...
class Zend_Mail_Protocol_Imap
{
    ...
    protected function _assumedNextLine($start)
    {
        ...
        return 0 === strpos((string) $line, $start);
    }
    ...
    public function readResponse($tag, $dontParse = false)
    {
        ...
        if ($dontParse) {
            ...
            $tokens = [substr((string) $tokens, 0, 2)];
        }
    ...
    public function sendRequest($command, $tokens = [], &$tag = null)
    {
        ...
        foreach ($tokens as $token) {
            if (is_array($token)) {
                if (false === @fputs($this->_socket, $line.' '.$token[0]."\r\n")) {
                    /*
                     * @see Zend_Mail_Protocol_Exception
                     */
                    // require_once 'Zend/Mail/Protocol/Exception.php';
        ...
        if (false === @fputs($this->_socket, $line."\r\n")) {
            /*
             * @see Zend_Mail_Protocol_Exception
             */
            // require_once 'Zend/Mail/Protocol/Exception.php';
    ...
    public function fetch($items, $from, $to = null)
    {
        ...
        while (!$this->readLine($tokens, $tag)) {
            ...
            if (null === $to && !is_array($from) && $tokens[0] != $from) {
                continue;
            }
