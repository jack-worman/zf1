...
class Zend_Currency
{
    ...
    public function __construct($options = null, $locale = null)
    {
        ...
        if ((null === $this->_options['currency']) and (null === $this->_options['name'])) {
        ...
        if ((is_array($calloptions) && !isset($calloptions['display']))
                || (!is_array($calloptions) && self::NO_SYMBOL == $this->_options['display'])) {
    ...
    public function toCurrency($value = null, array $options = [])
    {
        ...
        if (!(isset($value)) or (false === is_numeric($value))) {
            // require_once 'Zend/Currency/Exception.php';
        ...
        if (self::STANDARD !== $options['position']) {
            $value = str_replace((string) '¤', '', $value);
            $space = '';
            if (false !== iconv_strpos($value, ' ')) {
                $value = str_replace((string) ' ', '', $value);
                $space = ' ';
            }

            if (self::LEFT == $options['position']) {
                $value = '¤'.$space.$value;
            } else {
                $value = $value.$space.'¤';
            }
    ...
    private function _checkParams($currency = null, $locale = null)
    {
        ...
        if ((false === empty($currency)) and (false === empty($data))) {
    ...
    {
        if ((null === $currency) and (null === $locale)) {
    ...
    protected function _checkOptions(array $options = [])
    {
        ...
        foreach ($options as $name => $value) {
            ...
            switch ($name) {
                case 'position':
                    if ((self::STANDARD !== $value) and (self::RIGHT !== $value) and (self::LEFT !== $value)) {
                        // require_once 'Zend/Currency/Exception.php';
                ...
                case 'format':
                    if ((false === empty($value)) and (false === Zend_Locale::isLocale($value, null, false))) {
                        if (!is_string($value) || (false === strpos((string) $value, '0'))) {
                            // require_once 'Zend/Currency/Exception.php';
                            throw new Zend_Currency_Exception("'".(('object' === gettype($value)) ? get_class($value) : $value)."' is no format token");
                        }
                    }
                    break;

                case 'display':
                    if (is_numeric($value) and (self::NO_SYMBOL !== $value) and (self::USE_SYMBOL !== $value)
                        and (self::USE_SHORTNAME !== $value) and (self::USE_NAME !== $value)) {
                        // require_once 'Zend/Currency/Exception.php';
