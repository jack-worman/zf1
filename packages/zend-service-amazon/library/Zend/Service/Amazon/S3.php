...
class Zend_Service_Amazon_S3 extends Zend_Service_Amazon_Abstract
{
    ...
    public function getObjectStream($object, $streamfile = null, $paidobject = false)
    {
        ...
        if (200 != $response->getStatus() || !($response instanceof Zend_Http_Response_Stream)) {
            return false;
        }
    ...
    public function copyObject($sourceObject, $destObject, $meta = null)
    {
        ...
        $headers['x-amz-metadata-directive'] = null === $meta ? 'COPY' : 'REPLACE';

        $response = $this->_makeRequest('PUT', $destObject, null, $headers);

        if (200 == $response->getStatus() && !stristr($response->getBody(), '<Error>')) {
            return true;
        }
    ...
    public function _makeRequest($method, $path = '', $params = null, $headers = [], $data = null)
    {
        ...
        if (is_resource($data) && 'PUT' != $method) {
            /*
             * @see Zend_Service_Amazon_S3_Exception
             */
            // require_once 'Zend/Service/Amazon/S3/Exception.php';
    ...
    protected function addSignature($method, $path, &$headers)
    {
        ...
        if (false !== strpos((string) $path, '?location')) {
            ...
            $sig_str .= '?torrent';
        } elseif (false !== strpos((string) $path, '?versions')) {
            $sig_str .= '?versions';
        }
