...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface
{
    ...
    public static function send($var, $label = null, $style = null, $options = [])
    {
        ...
        if (self::DUMP != $meta['Type'] && $options['includeLineNumbers']) {
    ...
    protected function _getStackTrace($options)
    {
        ...
        if (isset($options['fixZendLogOffsetIfApplicable']) && $options['fixZendLogOffsetIfApplicable']) {
            if (count($trace) >= 3
                && isset($trace[0]['file']) && 'Log.php' == substr((string) $trace[0]['file'], -7, 7)
                && isset($trace[1]['function']) && '__call' == $trace[1]['function']) {
                $spliceOffset = 2;
    ...
    {
        if (!$this->_messages || self::PROTOCOL_URI != $protocolUri) {
            return;
        }
