...
abstract class Zend_Gdata_App_Base
{
    ...
    public function getDOM($doc = null, $majorVersion = 1, $minorVersion = null)
    {
        ...
        if (null != $this->_rootNamespaceURI) {
            $element = $doc->createElementNS($this->_rootNamespaceURI, $this->_rootElement);
        } elseif (null !== $this->_rootNamespace) {
            if (false === strpos((string) $this->_rootElement, ':')) {
        ...
        $minorVersion = null)
    {
        ...
        $key = $prefix.' '.
               (null === $majorVersion ? 'NULL' : $majorVersion).
               ' '.(null === $minorVersion ? 'NULL' : $minorVersion);
    ...
    public function __set($name, $val)
    {
        ...
        } elseif (isset($this->{'_'.$name}) || (null === $this->{'_'.$name})) {
            $this->{'_'.$name} = $val;
        } else {
            // require_once 'Zend/Gdata/App/InvalidArgumentException.php';
            throw new Zend_Gdata_App_InvalidArgumentException('Property '.$name.'  does not exist');
