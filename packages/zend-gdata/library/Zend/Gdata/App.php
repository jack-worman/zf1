...
class Zend_Gdata_App
{
     ...
     * @var Zend_Http_Client
     */
    protected static $_staticHttpClient;
        ...
        $contentTypeOverride = null)
    {
        ...
        if ('POST' != $method && 'GET' != $method && Zend_Gdata_App::getHttpMethodOverride()) {
        ...
        $body = null, $contentType = null, $remainingRedirects = null)
    {
        ...
        if (('POST' == $method || 'PUT' == $method) && null === $body
        ...
        if (Zend_Gdata_App::getGzipEnabled()) {
            ...
            if (false === strpos((string) $this->_httpClient->getHeader('User-Agent'),
                'gzip')) {
                $headers['User-Agent'] =
        ...
        if ($response->isRedirect() && '304' != $response->getStatus()) {
    ...
    public static function importFile($filename, $className = 'Zend_Gdata_App_Feed', $useIncludePath = false)
    {
        ...
        if (false === $feed) {
            ...
            // require_once 'Zend/Gdata/App/Exception.php';
            throw new Zend_Gdata_App_Exception("File could not be loaded: $phpErrormsg");
        }
    ...
    public function updateEntry($data, $uri = null, $className = null,
        $extraHeaders = [])
    {
        if (null === $className && $data instanceof Zend_Gdata_App_Entry) {
    ...
    public function generateIfMatchHeaderData($data, $allowWeek)
    {
        ...
        if ($this->_majorProtocolVersion >= 2
            ...
            $etag = $data->getEtag();
            if ((null !== $etag)
                    && ($allowWeek || 'W/' != substr((string) $etag, 0, 2))) {
                $result = $data->getEtag();
            }
        }
