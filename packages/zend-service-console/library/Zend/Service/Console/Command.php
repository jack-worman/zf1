...
class Zend_Service_Console_Command
{
    ...
    public static function bootstrap($argv)
    {
        ...
        if (defined('MICROSOFT_CONSOLE_COMMAND_HOST') && 'console' != strtolower((string) MICROSOFT_CONSOLE_COMMAND_HOST)) {
            return;
        }
    ...
    protected static function _buildModel()
    {
        ...
        foreach ($classes as $class) {
            ...
            if (0 == count($handlers) && $type->isSubclassOf('Zend_Service_Console_Command')) {
            ...
            for ($hi = 0; $hi < count($handlers); ++$hi) {
                ...
                foreach ($methods as $method) {
                    ...
                    if ('Command' == substr((string) $method->getName(), -7) && !in_array(substr((string) $method->getName(), 0, -7), $commands)) {
                    ...
                    if (count($commands) > 0) {
                        ...
                        for ($pi = 0; $pi < count($parameters); ++$pi) {
                            ...
                            if (is_null($parameterFor)) {
                                exit('@command-parameter-for missing for parameter $'.$parameter->getName());
                            }
                            ...
                            $parameterModel = (object) [
                                ...
                                'required' => (isset($parameterFor[3]) ? false !== strpos((string) strtolower((string) $parameterFor[3]), 'required') && false === strpos((string) strtolower((string) $parameterFor[3]), 'required if') : false),
                            ];
    ...
    protected static function _findValueForDocComment($docCommentName, $docComment)
    {
        ...
        foreach ($commentLines as $commentLine) {
            if (false !== strpos((string) $commentLine, $docCommentName.' ')) {
