...
class Zend_Service_WindowsAzure_SessionHandler
{
    ...
    public function open()
    {
        ...
        if (self::STORAGE_TYPE_TABLE == $this->_storageType) {
            $this->_storage->createTableIfNotExists($this->_sessionContainer);
        } elseif (self::STORAGE_TYPE_BLOB == $this->_storageType) {
    ...
    public function write($id, $serializedData)
    {
        ...
        if (strlen((string) $serializedData) >= self::MAX_TS_PROPERTY_SIZE && self::STORAGE_TYPE_TABLE == $this->_storageType) {
