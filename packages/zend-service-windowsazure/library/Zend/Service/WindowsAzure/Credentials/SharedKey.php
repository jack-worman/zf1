    ...
    public function signRequestHeaders(
        ...
        if (!is_null($headers)) {
            foreach ($headers as $header => $value) {
                ...
                if (Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PREFIX_STORAGE_HEADER == substr((string) $header, 0, strlen((string) Zend_Service_WindowsAzure_Credentials_CredentialsAbstract::PREFIX_STORAGE_HEADER))) {
        ...
        $contentLength = '';
        if (Zend_Http_Client::GET != strtoupper((string) $httpVerb)
             && Zend_Http_Client::DELETE != strtoupper((string) $httpVerb)
             && Zend_Http_Client::HEAD != strtoupper((string) $httpVerb)) {
            $contentLength = 0;
