...
class Zend_Service_WindowsAzure_Credentials_SharedAccessSignature extends Zend_Service_WindowsAzure_Credentials_CredentialsAbstract
{
    ...
    public function setPermissionSet($value = [])
    {
        foreach ($value as $url) {
            if (false === strpos((string) $url, $this->_accountName)) {
    ...
    public function permissionMatchesRequest(
        ...
        if (Zend_Service_WindowsAzure_Storage::RESOURCE_BLOB == $requiredResourceType) {
        ...
        foreach ($permissionParts as $part) {
            ...
            if ('sr' == $property) {
                $matches = $matches && (false !== strpbrk($value, $requiredResourceType));
            }

            if ('sp' == $property) {
                $matches = $matches && (false !== strpbrk($value, $requiredPermission));
            }
        }

        // Ok, but... does the resource match?
        $matches = $matches && (false !== strpos((string) $parsedRequestUrl['path'], $parsedPermissionUrl['path']));
    ...
    public function signRequestUrl(
        ...
        foreach ($this->getPermissionSet() as $permittedUrl) {
            if ($this->permissionMatchesRequest($permittedUrl, $requestUrl, $resourceType, $requiredPermission)) {
                ...
                if (false === strpos((string) $requestUrl, '?')) {
                    $requestUrl .= '?';
                } else {
                    $requestUrl .= '&';
                }
