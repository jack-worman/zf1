...
class Zend_Service_WindowsAzure_Storage_Table extends Zend_Service_WindowsAzure_Storage_BatchStorageAbstract
{
    ...
    public function retrieveEntities($tableName = '', $filter = '', $entityClass = 'Zend_Service_WindowsAzure_Storage_DynamicTableEntity', $nextPartitionKey = null, $nextRowKey = null)
    {
        ...
        if (is_string($tableName)) {
            ...
            // Append parentheses
            if (false === strpos((string) $tableName, '()')) {
                $tableName .= '()';
            }
        ...
        } elseif ('Zend_Service_WindowsAzure_Storage_TableEntityQuery' == get_class($tableName)) {
        ...
        // Perform request
        $response = null;
        if ($this->isInBatch() && 0 == $this->getCurrentBatch()->getOperationCount()) {
        ...
        if ($response->isSuccessful()) {
            ...
            if (!is_null($response->getHeader('x-ms-continuation-NextPartitionKey')) && !is_null($response->getHeader('x-ms-continuation-NextRowKey'))) {
                if (false === strpos((string) $queryString, '$top')) {
    ...
    protected function _generateAzureRepresentation(?Zend_Service_WindowsAzure_Storage_TableEntity $entity = null)
    {
        ...
        foreach ($azureValues as $azureValue) {
            ...
            if (!is_null($azureValue->Value)) {
                if ('edm.boolean' == strtolower((string) $azureValue->Type)) {
                    $value[] = (true == $azureValue->Value ? '1' : '0');
                } elseif ('edm.datetime' == strtolower((string) $azureValue->Type)) {
