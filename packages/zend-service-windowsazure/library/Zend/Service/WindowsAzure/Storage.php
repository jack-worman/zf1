...
class Zend_Service_WindowsAzure_Storage
{
    ...
    public function __construct(
        ...
        if (!$this->_usePathStyleUri
            && (self::URL_DEV_BLOB == $this->_host
                || self::URL_DEV_QUEUE == $this->_host
                || self::URL_DEV_TABLE == $this->_host)
        ) {
            // Local storage
    ...
    protected function _generateMetadataHeaders($metadata = [])
    {
        ...
        foreach ($metadata as $key => $value) {
            if (false !== strpos((string) $value, "\r") || false !== strpos((string) $value, "\n")) {
                // require_once 'Zend/Service/WindowsAzure/Exception.php';
    ...
    protected function _parseMetadataHeaders($headers = [])
    {
        ...
        foreach ($headers as $key => $value) {
            if ('x-ms-meta-' == substr((string) strtolower((string) $key), 0, 10)) {
    ...
    {
        if (0 === preg_match('/^[a-zA-Z0-9_@][a-zA-Z0-9_]*$/', $metadataName)) {
            return false;
        }
